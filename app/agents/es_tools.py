...
def get_stock_history(ticker: str, days: int = 30) -> str:
    """주어진 종목의 OHLCV 히스토리컬 데이터를 조회합니다.

    최근 N일간의 시가, 고가, 저가, 종가, 거래량 데이터를 반환합니다.
    주가 추세, 특정 기간 최고가/최저가, 거래량 분석 등에 활용합니다.

    지원 종목: AAPL, MSFT, TSLA, NVDA (이외 종목은 조회 불가)
    보유 데이터: 최근 1년치(최대 252 거래일)

    Args:
        ticker: 주식 티커 심볼. AAPL, MSFT, TSLA, NVDA 중 하나만 허용.
        days: 조회할 최근 거래일 수 (기본값: 30, 최대: 252)
    """
    try:
        ticker = ticker.upper()
